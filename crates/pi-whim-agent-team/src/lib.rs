...
pub struct AgentSupervisor {
    ...
    tool_audit_receiver: Option<std::sync::mpsc::Receiver<ToolAuditRecord>>,
...
impl AgentSupervisor {
    ...
    fn start_inner(
        ...
        let supervisor = Self {
            ...
            tool_audit_receiver: Some(tool_audit_receiver),
            stopping,
            server_thread,
        };
        observe_hook(
    ...
    pub fn take_tool_audit_events(&mut self) -> Option<std::sync::mpsc::Receiver<ToolAuditRecord>> {
        self.tool_audit_receiver.take()
    }
...
fn dispatch_request_cancellable(
        ...
        && let Err(error) = ensure_actor_active(host, actor_id)
    {
        ...
        record_tool_audit(
            ...
            &request.tool_name,
            actor_id,
            get_agent_level(host, actor_id),
            ToolAuditOutcome::DeniedByInactive,
        ...
        && let Err(error) = ensure_tool_enabled(host, actor_id, spec.name)
    {
        ...
        record_tool_audit(
            ...
            actor_id,
            get_agent_level(host, actor_id),
            ToolAuditOutcome::DeniedByPolicy,
            Some(format!("{}: {}", error.code, error.message)),
            Some(request.request_id.clone()),
            None,
            0,
    ...
    match (spec.handler)(host, actor_id, &request, cancelled) {
        Ok(content) => {
            ...
            record_tool_audit(
                ...
                None,
                Some(request.request_id.clone()),
                Some(tool_arguments_hash(&request.arguments)),
                0,
            );
...
fn spawn_agent_handler(
    ...
    spawn_agent(host, actor_id, &request.arguments, &request.request_id)
}
...
/// if the channel is full, the record is dropped (the supervisor never blocks
/// on compliance logging).
#[allow(clippy::too_many_arguments)]
pub(crate) fn record_tool_audit(
    host: &HostContext,
    tool: &str,
    agent_id: AgentId,
    agent_level: u8,
    ...
    let _ = host.tool_audit_sender.try_send(ToolAuditRecord {
        tool: tool.to_owned(),
...
/// Compute a SHA-256 hash of the tool arguments for audit logging.
/// This avoids storing plaintext arguments in the audit log.
pub(crate) fn tool_arguments_hash(arguments: &Value) -> String {
    use sha2::Digest;
    let encoded = serde_json::to_string(arguments).unwrap_or_default();
    format!("{:x}", sha2::Sha256::digest(encoded.as_bytes()))
}

/// Read the agent level from shared state. Returns 0 if unavailable.
pub(crate) fn get_agent_level(host: &HostContext, agent_id: AgentId) -> u8 {
    let (lock, _) = &*host.shared;
    lock.lock()
        .ok()
        .and_then(|state| {
...
mod tests {
    ...
    fn hook_payload_adds_stable_authenticated_agent_context() {
        ...
        assert_eq!(payload["agent_level"], descriptor.level);
        assert_eq!(payload["session_id"], descriptor.session_id.to_string());
        assert_eq!(payload["parent_agent_id"], Uuid::from_u128(4).to_string());
        assert_eq!(
            payload["parent_session_id"],
            descriptor.parent_session_id.unwrap().to_string()
        );
        assert_eq!(payload["request_id"], "request-42");
        assert_eq!(payload["agent_name"], "reviewer");
        assert_eq!(payload["agent_role"], "code review");
        assert_eq!(payload["tool"], LIST_AGENTS_TOOL);
    }

    #[test]
    fn hook_commands_receive_authenticated_agent_context() {
        ...
        let script = directory.path().join("verify-hook-context.sh");
        ...
        .unwrap();
        let capability = supervisor.root_capability.clone();
        let descriptor = file_actor(&supervisor.host, supervisor.root_id).unwrap();
        let checks = [
            "\"request_id\":\"hook-request\"".to_owned(),
            format!("\"team_id\":\"{}\"", descriptor.team_id),
            format!("\"session_id\":\"{}\"", descriptor.session_id),
            format!("\"agent_id\":\"{}\"", descriptor.id),
            "\"agent_level\":0".to_owned(),
            "\"parent_agent_id\":null".to_owned(),
            "\"parent_session_id\":null".to_owned(),
            "\"agent_name\":\"root\"".to_owned(),
            "\"agent_role\":\"team owner\"".to_owned(),
        ];
        let checks = checks
            .iter()
            .map(|check| {
                format!(
                    "printf '%s' \"$payload\" | /usr/bin/grep -F '{}' >/dev/null || exit 1",
                    check
                )
            })
            .collect::<Vec<_>>()
